...
class SeekBarView: UIView {
    ...
    private func handleSeeking(touch: UITouch) {
        ...
        let location = min(position.x, self.timeNotFillView.bounds.width)
        self.timeFillViewTrailingConstraint.constant = location
        self.layoutIfNeeded()
        delegate?.seekBarView(self, seekTimeProgress: location/self.timeNotFillView.bounds.width)
    }
}
